...
class TCPState {
  ...
  public:
    ...
    enum class State {  // TCP 状态机
        LISTEN = 0,     //!< Listening for a peer to connect 监听
        SYN_RCVD,       //!< Got the peer's SYN 收到了第一个 SYN 包
        SYN_SENT,       //!< Sent a SYN to initiate a connection 发送了 SYN 包
        ESTABLISHED,    //!< Three-way handshake complete 建立连接，三次握手成功
